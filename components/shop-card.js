...
const ShopCard = ({ data }) => {
  return (
    <Link href={`/explore-campaign/${data?.slug}`}>
      <div className="shop-card w-full border border-[#E4E4E7] rounded-2xl p-5 md:py-8 md:px-10 bg-[#ffffff] group">
        <div className="w-full flex flex-col md:flex-row justify-center md:gap-5 md:justify-between md:items-center ">
          <div className="flex justify-center h-[120px] lg:h-[253px] mb-6 md:mb-0 ">
            <div className="w-[120px] h-full lg:w-[186px] rounded-lg md:rounded-xl overflow-hidden">
              <Image
                src={data?.product_images[0]?.file_url}
                width={186}
                height={253}
                alt={data?.product_images[0]?.file_name}
                className="w-full h-full lg:w-[186px] lg:h-[253px] object-contain"
              />
            </div>
          </div>
          {/* content */}
